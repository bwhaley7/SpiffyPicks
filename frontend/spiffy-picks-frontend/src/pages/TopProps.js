...
const TopProps = () => {
  ...
  useEffect(() => {
    const fetchTopProps = async () => {
      try {
        const response = await axios.get('http://localhost:5200/api/picks/props/toppicks'); // Adjust the endpoint
  ...
  return (
    <main>
      <section id="top-props">
        <h2>Top Props</h2>

        {/* Grid for Top Props */}
        <div className="pick-grid">
          {topProps.map((prop, index) => (
            <div className="pick-card" key={index} style={{ backgroundColor: teamColors[prop.team]?.secondary }}>
